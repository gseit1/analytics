...
<template>
  <div class="page-content dashboard-page">
    <div class="dashboard-header px-4 py-4 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between mb-4">
      <div class="d-flex align-items-center mb-3 mb-md-0">
        <i class="bi bi-speedometer2 dashboard-icon me-3"></i>
        <div>
    ...
    <div v-else>
      <!-- Stats Cards -->
      <div class="dashboard-stats-cards d-flex flex-wrap gap-3 mb-4">
        ...
        <div class="card stats-card info flex-fill">
          <div class="card-body">
            <div class="d-flex justify-content-between">
              <div>
        ...
        <div class="card stats-card warning flex-fill">
          <div class="card-body">
            <div class="d-flex justify-content-between">
              <div>
      ...
      <!-- Net Income Row -->
      <div class="dashboard-net-income d-flex flex-wrap gap-3 mb-4">
        <div class="card stats-card flex-fill" :class="netIncomeMonthClass">
          <div class="card-body">
            <div class="d-flex justify-content-between">
              <div>
                <div class="stats-label">Net Income (This Month)</div>
                <div class="stats-value">€{{ formatEuroAmount(netIncomeMonth) }}</div>
              </div>
              <div :class="`text-${netIncomeMonthClass}`">
        ...
        <div class="card stats-card info flex-fill">
          <div class="card-body">
            <div class="d-flex justify-content-between">
              <div>
                <div class="stats-label">Net Income to Date</div>
                <div class="stats-value">€{{ formatEuroAmount(netIncomeToDate) }}</div>
              </div>
              <div class="text-info">
...
export default {
  ...
  setup () {
    ...
    const netIncomeMonth = computed(() => {
      const work = dashboardData.value.currentMonth?.work?.total_earnings || 0
      const exp = dashboardData.value.currentMonth?.expenses || { income: 0, expense: 0 }
      return (work + (exp.income || 0) - (exp.expense || 0)).toFixed(2)
    })

    const netIncomeMonthClass = computed(() => {
      const net = parseFloat(netIncomeMonth.value)
      return net >= 0 ? 'success' : 'danger'
    })

    const netIncomeToDate = computed(() => {
      // prefer backend computed net if provided
      const backendNet = dashboardData.value.toDate?.net
      if (typeof backendNet === 'number') return backendNet.toFixed(2)
      const work = dashboardData.value.toDate?.work?.total_earnings || 0
      const exp = dashboardData.value.toDate?.expenses || { income: 0, expense: 0 }
      return (work + (exp.income || 0) - (exp.expense || 0)).toFixed(2)
    })
